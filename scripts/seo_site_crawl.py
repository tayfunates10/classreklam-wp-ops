...
class Parser(HTMLParser):
    ...
    def handle_endtag(self, tag):
        ...
        if tag == 'title' and self._in_title:
            ...
            if self._h1_depth == 1:
                self.h1.append(clean(' '.join(self._h1_buf)))
            self._h1_depth -= 1
...
def canonicalize(raw, base=BASE + '/'):
    ...
    if p.scheme not in ('http', 'https'):
        return None
    if (p.hostname or '').lower() not in ('classreklamtabela.com.tr', 'www.classreklamtabela.com.tr'):
        return None
    path = p.path or '/'
    if path.startswith(EXCLUDE_PREFIXES):
        return None
    if re.search(r'\.(?:jpg|jpeg|png|gif|webp|avif|svg|pdf|zip|css|js|xml|ico|woff2?|ttf|eot)$', path, re.I):
        return None
...
def sitemap_urls():
    discovered = set()
    evidence = []
    queue = collections.deque([BASE + '/sitemap_index.xml'])
    seen_maps = set()
...
def main():
    ...
    ap.add_argument('--output', default='.ops/seo-site-crawl-2026-08-14.json')
    args = ap.parse_args()

    seeds, sm_evidence = sitemap_urls()
    seeds.add(BASE + '/')
    ...
    while q and len(seen) < MAX_URLS:
        ...
        record = {
            ...
            'canonical': canonicalize(parser.canonical, url) if parser.canonical else '',
            'robots': clean(parser.robots),
            'h1': [x for x in parser.h1 if x],
            'internal_links': [],
            'error': res.get('error', ''),
        }
        links = []
        for href in parser.links:
            ...
            if child:
                links.append(child)
                incoming[child].add(url)
                if child not in seen and child not in q and len(seen) + len(q) < MAX_URLS:
                    q.append(child)
        record['internal_links'] = sorted(set(links))
        pages[url] = record
    ...
    for url, rec in pages.items():
        if rec['classification'] != 'INDEX':
            continue
        if rec['title']:
            title_map[rec['title'].casefold()].append(url)
        if rec['description']:
            desc_map[rec['description'].casefold()].append(url)

    report = {
        'checked_at': dt.datetime.now(dt.timezone.utc).isoformat(),
        'base': BASE,
        'limits': {'max_urls': MAX_URLS, 'delay_seconds': DELAY},
        'sitemaps': sm_evidence,
        'sitemap_seed_count': len(seeds),
        'crawled_count': len(pages),
        'waf_urls': waf_urls,
        'broken_internal_edges': broken_edges,
        'duplicate_titles': [v for v in title_map.values() if len(v) > 1],
        'duplicate_descriptions': [v for v in desc_map.values() if len(v) > 1],
