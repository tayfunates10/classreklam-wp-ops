...
TARGET_MARKER='@keyframes cr-solutions-white-breathe'
...
def live_check(label):
    ...
    url=BASE+'/?crpurge='+stamp+'&phase='+urllib.parse.quote(label)
    ...
    return {
        ...
        'alternate_present':'animation-direction: alternate !important' in html,
...
try:
    ...
    if ls:
        plugin_file=ls[0]['plugin']
        initial_status=ls[0].get('status','inactive')
        ...
        if not plugin_file:
            raise RuntimeError(f'LiteSpeed Cache install did not return plugin id: {body}')
        created=True
    ...
    # Resave homepage after LiteSpeed integration is active. LSCWP hooks into post updates and purges related cache entries.
    code,page,_=request('GET','/wp/v2/pages/6',params={'context':'edit','_fields':'id,content,modified'})
    raw=page.get('content',{}).get('raw','')
    if TARGET_MARKER not in raw:
        raise RuntimeError('Authoritative homepage content does not contain the new animation marker')
    ...
    if not result['after_purge_live']['marker_present']:
        # One second resave/purge attempt before rollback.
        code,updated,_=request('POST','/wp/v2/pages/6',payload={'content':raw})
        result['steps'].append({'second_resave_http':code})
        time.sleep(3)
        result['after_second_purge_live']=live_check('active2')

finally:
    # Restore plugin state exactly as it was before this maintenance operation.
    if plugin_file:
        try:
            if created:
                ...
                result['steps'].append({'plugin_restore':'left-active-as-found'})
        except Exception as e:
            result['steps'].append({'plugin_restore_error':str(e)[:800]})

try:
    time.sleep(2)
    result['final_live']=live_check('final')
except Exception as e:
    result['final_live_error']=str(e)[:1000]

ok=bool(result.get('final_live',{}).get('marker_present'))
